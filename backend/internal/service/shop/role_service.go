...
func NewRoleService(roleRepo shop.RoleRepository) *RoleService {
...
func (s *RoleService) Delete(db *gorm.DB, id, tenantID uint64) error {
	...
	if err := s.roleRepo.Delete(db, id, tenantID); err != nil {
		return err
	}
	middleware.InvalidateRolePermsCache(db, id, tenantID)
	return nil
}
