...
func (ctrl *AuthController) Login(c *gin.Context) {
	...
	resp, err := ctrl.svc.ShopLogin(req)
	if err != nil {
...
func (ctrl *AuthController) Permissions(c *gin.Context) {
	tree, err := ctrl.svc.GetPermissionTree(enum.SystemTypeShop)
	if err != nil {
		response.InternalError(c, err.Error())
		return
	}
...
func RegisterShopAuthRoutes(rg *gin.RouterGroup, ctrl *AuthController) {
	auth := rg.Group("/auth")
	{
