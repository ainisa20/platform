...
type UserRepository interface {
	...
	UpdatePassword(db *gorm.DB, userID uint64, hashedPassword string) error
}
...
func (r *userRepository) UpdatePassword(db *gorm.DB, userID uint64, hashedPassword string) error {
	return db.Model(&entity.SysUser{}).Where("id = ?", userID).Update("password", hashedPassword).Error
}
